...
class TextLabel:
    def __init__(self, canvas, color):
        self.canvas = canvas
        self.id = canvas.create_text(350, 200, text=text, fill=color,
                                     font=('Times', fontsize), state='hidden')
        
    def show(self):
        self.canvas.itemconfig(self.id, state='normal')
        canvas.itemconfig(game_ovar_text, state='normal')
...
game_over_text = canvas.create_text(250, 200, text='G A M E       O V E R', state='hidden')
game_clear_text = canvas.create_text(250, 200, text='clear', state='hidden')
...
while True:
    if  ball.hit_bottom == True:
        ...
        break
         ...
         time.sleep(1)
         canvas.itemconfig(game_clear_text, state='normal')
    else: 
          ball.draw()
          paddle.draw()
